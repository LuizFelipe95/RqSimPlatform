...
public static class DispatcherConfig
{
    ...
    /// Set high enough to survive console graph reinitialization (GPU init + graph build).
    /// </summary>
    public static TimeSpan StaleDataThreshold { get; } = TimeSpan.FromSeconds(10);
    ...
    /// Validates that the companion RqSimConsole.dll exists alongside the exe,
    /// since .NET publishes framework-dependent apps as exe+dll pairs.
    /// </summary>
    private static string FindSimulationExecutable()
    {
        const string exeName = "RqSimConsole.exe";
        const string dllName = "RqSimConsole.dll";
        var baseDir = AppContext.BaseDirectory;

        // 1. Same directory (deployed scenario - side-by-side)
        // Validate companion DLL exists to avoid stale exe copies missing dependencies.
        var sameDir = Path.Combine(baseDir, exeName);
        if (File.Exists(sameDir) && File.Exists(Path.Combine(baseDir, dllName)))
        {
        ...
        {
            Trace.WriteLine($"[DispatcherConfig] Found stale {exeName} in same directory (missing {dllName}), skipping");
        }
