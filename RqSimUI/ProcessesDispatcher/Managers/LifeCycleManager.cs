...
public sealed class LifeCycleManager : IDisposable
{
    ...
    public async Task OnFormLoadAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Wait for console to initialize GPU cluster, pipe server, and shared memory.
            await Task.Delay(2000, cancellationToken).ConfigureAwait(false);

            bool handshakeOk = await _ipc.SendHandshakeWithRetryAsync(maxRetries: 5, cancellationToken).ConfigureAwait(false);
            if (handshakeOk)
            {
                Log("[LifeCycleManager] Console process started and handshake successful.");
            }
            else
            {
                Log("[LifeCycleManager] Console started but handshake failed after retries — will retry on bind.");
            }
        }
        catch (FileNotFoundException ex)
        {
            // Console executable not found - continue in standalone mode
            Log($"[LifeCycleManager] Running in standalone mode: {ex.Message}");
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            // Log but don't fail - form can work standalone
            Log($"[LifeCycleManager] Console process startup failed: {ex.Message}");
        }
    }
    ...
    /// <summary>
    /// Gets render nodes from shared memory for external simulation visualization.
    /// Returns null if not connected or no data available.
    /// </summary>
    public RenderNode[]? GetExternalRenderNodes()
    {
        return _stateSynchronizer.GetRenderNodes();
    }

    /// <summary>
    /// Gets render edges from shared memory for external simulation visualization.
    /// Returns null if not connected or no edge data available.
    /// </summary>
    public RenderEdge[]? GetExternalRenderEdges()
    {
        return _stateSynchronizer.GetRenderEdges();
    }

    /// <summary>
    /// Gets node count from shared memory without reading full node data.
    /// </summary>
    public int GetExternalNodeCount()
    {
        return _stateSynchronizer.GetNodeCount();
    }

    public async Task<bool> HandleClosingAsync(CloseReason closeReason, CancellationToken cancellationToken = default)
    {
        ...
        var result = MessageBox.Show(
            "Terminate simulation process?\n\nYes - Shutdown and close.\nNo - Detach (leave running).\nCancel - Do not close.",
        ...
        {
            // Detach: UI closes, process continues
            _dispatcher.Detach();
        }
        return true;
    }
