...
public sealed class LifeCycleManager : IDisposable
{
    ...
    public async Task OnFormLoadAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            Log("[LifeCycleManager] Console process started and handshake sent.");
        }
        catch (FileNotFoundException ex)
        {
            // Console executable not found - continue in standalone mode
            Log($"[LifeCycleManager] Running in standalone mode: {ex.Message}");
            // Form will work directly with RqSimEngineAPI without external process
        }
    ...
    /// <summary>
    /// Gets pipeline module stats from shared memory.
    /// Returns empty array if not connected or no module stats available.
    /// </summary>
    public ModuleStatsEntry[] GetExternalModuleStats()
    {
        return _stateSynchronizer.GetModuleStats();
    }
